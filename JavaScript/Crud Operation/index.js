...
document.body.onload = getIdentity();

//Get Data From Server (D.json)
async function getIdentity() {
    let response;
    try {
        response = await fetch('http://localhost:3000/Identity');
...
function createTableBody(Identity) {
    ...
    Identity.forEach(element => {
        ...
        tableHeaders.forEach(header => {
            ...
            if (header.action !== 'Action') {
                ...
                editBtn.onclick = function () {editIdentity(element)};
                td.appendChild(document.innerHTML = editBtn);

                const deleteBtn = document.createElement('button');
                deleteBtn.innerText = 'Delete';
                deleteBtn.onclick = function () {deleteIdentity(element)};
...
//Update Data
var submitButton = document.getElementById('editIdentity');
function editIdentity(element) {
    ...
    submitBtn.onclick = async function () {
        ...
        fetch(`http://localhost:3000/Identity/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
